...
> [!NOTE] > `typeof null` will return `object` instead of `null`. This behaviour
...
> [!NOTE] > `-Infinity`, `+Infinity` and even `NaN` (not a number) are a special
...
> [!NOTE] Unlike `let` or `const`, `var` variables can be redeclared in the same
...
> [!NOTE] We can say `const` and `let` are in temporal dead zone from the start
